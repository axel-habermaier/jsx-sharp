...
/// <summary>
///     <para>
///         A bridge to support MVC's model binders in minimal APIs. Specify <see cref="ModelBinder{T}" /> as a paramter
///         type
///         of your method to trigger the model binding system.
///     </para>
///     <para>
///         This requires registering the model binding services by calling
///         <see cref="MvcCoreServiceCollectionExtensions.AddMvcCore(IServiceCollection)" /> or
...
public class ModelBinder<TValue>
{
    ...
    /// <summary>
    ///     Binds the specified parameter from <see cref="HttpContext.Request" />. This method is called by the framework on
    ///     your behalf
    ...
    public static async ValueTask<ModelBinder<TValue>> BindAsync(HttpContext context, ParameterInfo parameter)
    {
        ...
        var (binder, bindingInfo, metadata) = _cache.GetOrAdd((parameter, modelBinderFactory, modelMetadataProvider),
            static arg =>
            {
                var (parameter, modelBinderFactory, modelMetadataProvider) = arg;

                var metadata = modelMetadataProvider.GetMetadataForType(typeof(TValue));
        ...
        var valueProviderFactories = context.RequestServices.GetRequiredService<IOptions<MvcOptions>>().Value
        ...
        var result = await parameterBinder.BindModelAsync(actionContext, binder, valueProvider, paramterDescriptor,
            metadata, value: null, container: null);
